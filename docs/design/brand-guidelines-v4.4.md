# CALLVAULT BRAND GUIDELINES v4.4
...
<p className="text-muted-foreground">Tertiary text, icons</p>

{/* Borders */}
<div className="border border-border">Standard border</div>
<div className="border border-border/40">Subtle border</div>

{/* Hover states */}
<tr className="hover:bg-muted">Hoverable row</tr>
```
...
### v2 AppShell — 4-Pane Architecture

**Note:** This section documents the v2 layout system introduced in Phase 16 (January 2026). The Card-on-Viewport system above describes the v1 layout pattern, which may still appear in legacy routes.

The v2 app uses a 4-pane spring-animated layout managed by `AppShell.tsx`:

```
┌─────────────────────────────────────────────────────────────┐
│  OrgSwitcherBar (bg-viewport, full width, no border-b)      │
├──────────┬──────────────┬──────────────────┬────────────────┤
│  Pane 1  │    Pane 2    │     Pane 3       │    Pane 4      │
│ Nav Rail │  Secondary   │  Main Content    │  Detail Panel  │
│ 72–220px │   0–280px    │   flex-1 grow    │   0–360px      │
│ animated │   animated   │   always shown   │   animated     │
└──────────┴──────────────┴──────────────────┴────────────────┘
```

**Pane widths (locked values from AppShell.tsx):**

| Pane | Expanded | Collapsed |
|------|----------|-----------|
| Pane 1 (Nav) | 220px | 72px |
| Pane 2 (Secondary) | 280px | 0px (hidden) |
| Pane 3 (Main) | flex-1 | flex-1 |
| Pane 4 (Detail) | 360px desktop / 320px tablet | 0px (hidden) |
...
<div className="h-full flex flex-col overflow-hidden bg-viewport">
  ...
  <div className="flex-1 flex gap-3 overflow-hidden p-1 min-h-0">
    {/* All 4 panes as siblings in flex row */}
  </div>
</div>
...
All pane width changes use spring physics, not CSS transitions:

```typescript
// In AppShell.tsx and DetailPaneOutlet.tsx — do not change
const SPRING = { type: 'spring', stiffness: 260, damping: 28 } as const
```

**OrgSwitcherBar shell chrome rule:**

OrgSwitcherBar uses `bg-viewport` (not `bg-card`). It is shell chrome, not a pane. No `border-b`, no `backdrop-blur`. The `p-1` gap between bar and panes provides visual separation.

**DetailPaneOutlet width-only animation:**

Pane 4 animates width only (same-plane push into Pane 3). No `x` offset — that creates a drawer-slide feel:
...
// Wrong: adds x offset (drawer feel)
initial={{ width: 0, x: 20, opacity: 0 }}
```

**Close button in detail panes:**

Close button must be in a flex-flow header row. Never `absolute` inside `overflow-hidden` — it gets clipped:

```tsx
// Correct: flex-flow header
...
| Icon color (light) | `text-foreground` |
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        // Background and text
        'bg-white text-foreground dark:bg-black dark:text-white',
...
border: 1px solid #E8E8E8;  /* border-soft → V2: border-border/40 */
border-radius: 8px;
```
...
```css
background: transparent;
color: #9A9A9A;  /* muted-foreground in dark mode */
...
### Dialog Action Buttons (Exception)

Inside Radix Dialog forms, the brand-500/brand-600 color scale is an accepted alternative for primary submit actions. This exception exists because dialogs have a contained visual context where the branded color reads as affirmative rather than decorative.

```tsx
{/* ACCEPTED: Inside Radix Dialog only */}
<Button
  type="submit"
...
{/* ALSO ACCEPTED: Linear-style adaptive CTA */}
<Button className="bg-foreground text-background hover:bg-foreground/90">
  Create Workspace
</Button>
```

**Where this exception applies:** Radix Dialog content only (create, edit, confirm dialogs).
**Where it does NOT apply:** Page-level buttons, table row actions, toolbars, any non-dialog context.
...
**Color:** Icons use `text-muted-foreground` (#7A7A7A light / #6B6B6B dark)
...
<span className="flex items-center gap-2">
  <RiCheckLine className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
  Completed
</span>
```
...
|-------|------|------|--------|-------|
| **Inactive** | Inter | 14px | Light (300) | #7A7A7A (muted-foreground) |
...
### Custom Button-Based Tabs (Alternative Pattern)

When page-level tabs require custom interaction (e.g., the Import page Sources/Rules tabs) and Radix Tabs doesn't provide required control, use custom button tabs with the same vibe orange underline treatment.

**Implementation (from `import/index.tsx`):**

```tsx
{/* Tab bar container */}
<div className="flex gap-6 border-b border-border px-4 pt-3">
  {(['sources', 'rules'] as const).map((tab) => (
    <button
      key={tab}
      onClick={() => setActiveTab(tab)}
      className={cn(
        'px-1 pb-2 text-sm font-medium transition-colors capitalize',
        activeTab === tab
          ? 'border-b-2 border-vibe-orange text-foreground'
...
**Active state:** `border-b-2 border-vibe-orange text-foreground`
**Inactive state:** `text-muted-foreground hover:text-foreground`

Use this pattern when you need controlled tab state outside of Radix Tabs (custom animations, conditional loading, etc.). Use the Radix `<Tabs>` component when you want accessible tab panel management.
...
|-----------|---------------|--------------|
| **Pill Indicator** | 4px wide, 65–80% height, flush left | `absolute left-0 w-1 h-[65%] rounded-r-full bg-vibe-orange` |
...
|----------|-------|
| Width | 4px (`w-1`) |
| Height | 65–80% of parent (`h-[65%]` recommended) |
...
**Height Note (Phase 16.1 audit):** The v2 SidebarNav implementation uses `h-[65%]` (slightly shorter, more proportional with the nav item height). `h-[80%]` remains acceptable — choose based on visual balance with the nav item size.
...
```tsx
className="bg-muted text-foreground rounded-2xl p-3 border border-border/40"
```

- Light Mode: #F8F8F8 background (bg-muted)
- Dark Mode: #2A2A2A background
- Text: #111111 (text-foreground)
- Border: Optional 1px border-border/40 for extra definition
- Border-radius: 16px (rounded-2xl)
...
1. **Fast** - Nothing over 300ms for CSS transitions
2. **Subtle** - Confirm actions, don't distract
3. **Purposeful** - Only animate meaningful state changes
4. **Spring-native** - Use spring physics for layout changes (pane width, panel enter/exit)

### v2 Spring Animation System

The v2 app uses `motion/react` (not `framer-motion`) for all animated layout changes.

**Import (CRITICAL — always use `motion/react`, never `framer-motion`):**

```tsx
import { motion, AnimatePresence } from 'motion/react'
```

**Spring config (locked — do not change):**

```typescript
const SPRING = { type: 'spring', stiffness: 260, damping: 28 } as const
```

**When to use spring vs. CSS transition:**

| Context | System | Why |
|---------|--------|-----|
| Pane width changes | Spring (motion/react) | Physical feel matches desktop app |
| Panel enter/exit | Spring + AnimatePresence | Coordinated enter/exit via AnimatePresence |
| Hover states | CSS transition (150ms) | Too fast for spring overhead |
| Button press | CSS transition (100ms) | Immediate feel |
| Modal open/close | CSS transition or spring | Either works |

**Panel enter/exit pattern:**
...
:root {
  ...
  --border-soft: 0 0% 95%;                 /* #F2F2F2 - V2: use border-border/40 class */
...
colors: {
  ...
  white: "hsl(var(--white))",
  black: "hsl(var(--black))",
  // V2: use text-foreground instead of text-ink
  ink: "hsl(var(--ink))",              /* V2: foreground */
  "ink-soft": "hsl(var(--ink-soft))",  /* V2: muted-foreground */
  "ink-muted": "hsl(var(--ink-muted))",/* V2: muted-foreground */
  border: "hsl(var(--border))",
  "border-soft": "hsl(var(--border-soft))",   /* V2: use border-border/40 class */
...
<p className="text-foreground">Primary text</p>
<p className="text-muted-foreground">Secondary text</p>
<p className="text-muted-foreground">Tertiary/muted text</p>

// Backgrounds
<div className="bg-viewport">Viewport background</div>
<div className="bg-card">Card background</div>
<div className="bg-muted">Hover state</div>

// Borders
<div className="border border-border">Standard border</div>
<div className="border border-border/40">Light divider</div>
...
<p className="text-foreground dark:text-text-dark-primary">Adapts to theme</p>
```
...
- Added v2 AppShell 4-pane architecture documentation (Phase 16 layout system)
- Added spring animation system documentation (`motion/react`, stiffness 260 damping 28)
- Updated Navigation pill indicator height to 65% (from 80%) to match v2 implementation
- Added custom button-based tab pattern (Import page pattern)
- Added dialog action button exception (bg-brand-500 accepted inside Radix Dialog)
...
*END OF BRAND GUIDELINES v4.4*

This document is complete and accurate as of March 1, 2026.

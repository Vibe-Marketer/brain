...
Every new account starts with an organization named **"Personal."** This works fine for solo use, but if you plan to invite teammates, rename it to something recognizable — like your company or team name — before sending invites.

- **Owners and admins** can rename the organization from **Organization → Overview**.
- **Members** see the name as read-only. Contact your organization admin to request a change.

CallVault nudges owners to rename once additional members join, and the invite dialog warns you if the org is still called "Personal."

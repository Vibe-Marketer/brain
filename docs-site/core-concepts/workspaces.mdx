...
When you import a call — whether from YouTube, a file upload, or an integration like Fathom or Zoom — you can choose which workspace it lands in. Import forms include a **Destination workspace** dropdown that defaults to your My Calls workspace but lets you select any workspace you have access to.

CallVault remembers your last-used workspace per import source, so repeated imports go to the same place without extra clicks.

If you need more advanced routing (e.g., based on meeting title or participants), set up [Routing Rules](/automation/routing-rules) to organize calls automatically.

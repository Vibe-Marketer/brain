...
Below your workspaces in the sidebar, you'll find the **Shared With Me** section. Click it to see every call that other people have shared with you across the organization.

Shared With Me acts like a virtual workspace — it replaces the main call list with only the calls shared with you, so you can review them without navigating away from your usual workflow. Selecting any workspace or clicking **Home** in the sidebar exits the shared view and returns you to your own calls.

<Note>
  Shared With Me only shows calls that others have explicitly shared with you. It does not include calls from workspaces you belong to — those appear in their respective workspaces.
</Note>

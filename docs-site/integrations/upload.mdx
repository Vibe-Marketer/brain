...
The upload form includes a **Destination workspace** dropdown where you choose which workspace the uploaded call lands in. Your personal My Calls workspace is the default, but you can select any workspace you have access to.

CallVault remembers your last-used workspace for file uploads, so the next time you upload a recording the same workspace is pre-selected.

You can also use [Routing Rules](/automation/routing-rules) to automatically organize uploaded calls based on title or other conditions.

...
<Info>
  The Zoom integration is **coming soon**. You can still bring Zoom recordings into CallVault today by downloading them from Zoom and using [file upload](/integrations/upload).
</Info>

When the Zoom integration launches, your cloud recordings will flow in automatically after each meeting ends — no downloads, no manual uploads.

<Note>
  This integration will work with **Zoom cloud recordings**. Local recordings saved to your computer will not sync automatically — upload those manually via [file upload](/integrations/upload).
</Note>

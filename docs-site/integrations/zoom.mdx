...
- **Date and duration**
- **Host** — the meeting host is identified and shown in the transcript view (blue bubbles on the right side)
- **Participants** — speaker names are automatically extracted from the transcript and listed as call participants

CallVault then transcribes the recording and generates an [AI summary](/transcripts/ai-summaries) — including key topics, action items, and highlights — without any extra steps.
...
<AccordionGroup>
  ...
  <Accordion title="Why don't Zoom calls show an invitee count?">
    Zoom doesn't provide calendar invitee data through its API, so the "Number of Invitees" field is hidden for Zoom calls. You'll still see the list of participants who actually spoke during the call.
  </Accordion>
</AccordionGroup>

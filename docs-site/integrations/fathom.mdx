...
New calls from Fathom land in your **My Calls** workspace by default. You can change the default destination during initial setup by choosing a different workspace in the import settings.

To automatically route calls to specific workspaces or folders based on meeting title, participants, or duration, set up [Routing Rules](/automation/routing-rules).
...
<AccordionGroup>
  ...
  <Accordion title="I want calls to go to a specific workspace, not My Calls">
    You can change the default destination workspace in your Fathom import settings. For more granular control, set up a [Routing Rule](/automation/routing-rules) with a condition matching your meeting titles and an action to assign the target workspace and folder.
  </Accordion>
</AccordionGroup>

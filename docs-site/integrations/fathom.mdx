...
If you're on a **Starter**, **Business**, or **Enterprise** plan, you can connect more than one Fathom account to CallVault. This is useful when you have separate Fathom accounts for different teams or clients.

To add another account:

1. Go to **Settings → Integrations → Fathom**
2. Click **Add Account**
3. Sign in to the additional Fathom account and authorize access

Each connected account syncs independently. Calls from all accounts land in **My Calls** by default — use [Routing Rules](/automation/routing-rules) to sort them into different workspaces automatically.

<Note>
  Free plan users can connect one Fathom account. Upgrade to a paid plan to connect additional accounts.
</Note>

If you reconnect a Fathom account that is already linked, CallVault automatically detects the duplicate and refreshes the existing connection instead of creating a second entry.

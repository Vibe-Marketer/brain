-- Migration: Phase 19 unconditional auto-demotion audit
-- Purpose: H-1 — audit every rollup-driven auto-to-manual rung transition.
-- Date: 2026-06-13
...
BEGIN
  ...
  existing_before AS (
    ...
    JOIN categories c ON c.category = act.category
    FOR UPDATE OF act
  ),
  upserted AS (
    ...
    LEFT JOIN existing_before existing ON existing.category = c.category
  ...
  FROM upserted u
  JOIN existing_before old ON old.category = u.category
  WHERE old.rung = 'auto'
    AND u.rung = 'manual'
    AND NOT EXISTS (
      SELECT 1
...
COMMENT ON FUNCTION public.rollup_autopilot_category_trust() IS
  'Persists category survival counters from matured merged runner_runs. Pending rows mature to held after survival_due_at when no reopen event exists; rollup-driven auto-to-manual transitions always emit auto_demoted audit events.';

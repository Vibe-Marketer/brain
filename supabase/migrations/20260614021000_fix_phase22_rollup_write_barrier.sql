-- Phase 22 review fix follow-up: force the rollup side-effect CTEs while
-- preserving the already-applied typed landed-signal, honest grouping, and
-- threshold-column fixes.
...
BEGIN
  ...
  write_barriers AS (
    SELECT
      ...
      (SELECT COUNT(*) FROM landed) AS landed_count
  ),
  ...
  SET
    ...
    updated_at = now()
  FROM updated
  FULL JOIN write_barriers ON true

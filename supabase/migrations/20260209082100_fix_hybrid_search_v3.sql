-- Fix hybrid_search_transcripts function - add type casts for double precision
...
BEGIN
    ...
    full_text_search AS (
        SELECT 
            tc.id,
            tc.recording_id,
            ts_rank_cd(to_tsvector('english', tc.chunk_text), plainto_tsquery('english', query_text))::double precision AS rank
        FROM transcript_chunks tc
        JOIN fathom_calls fc ON tc.recording_id = fc.recording_id
        LEFT JOIN recordings r ON r.legacy_recording_id = fc.recording_id
        WHERE 

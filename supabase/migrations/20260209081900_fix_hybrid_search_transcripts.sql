-- Fix hybrid_search_transcripts function
-- Issues fixed:
-- 1. fc.id -> fc.recording_id (fathom_calls uses recording_id, not id)
-- 2. fc.call_date -> fc.created_at (fathom_calls uses created_at)
-- 3. fc.bank_id -> join through recordings table to get bank_id

DROP FUNCTION IF EXISTS hybrid_search_transcripts(text, vector, integer, double precision, double precision, integer, uuid, timestamp with time zone, timestamp with time zone, text[], text[], bigint[], text[], text, text[], text[], text[], uuid);
...
BEGIN
    ...
    WITH semantic_search AS (
        ...
        WHERE 
            ...
            AND (filter_date_start IS NULL OR fc.created_at >= filter_date_start)
            AND (filter_date_end IS NULL OR fc.created_at <= filter_date_end)

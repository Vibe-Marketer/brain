-- Migration: Preserve event identity across current cross-org copy/move functions
-- Based on: 20260730160000_fix_cross_org_copy_dedup.sql
...
-- Phase 38 change: copy the exact source event_id into every newly inserted
-- destination recording while leaving access-policy columns to the destination
-- owner's INSERT trigger. All dedup, membership, workspace, timeout, and delete
...
BEGIN
  ...
  IF NOT public.is_organization_member(v_source.organization_id, v_caller_id) THEN
    RAISE EXCEPTION 'Access denied: not a member of source organization';
  END IF;

  -- SECURITY: caller must be member of TARGET organization
  IF NOT public.is_organization_member(p_target_org_id, v_caller_id) THEN
  ...
  IF NOT public.is_workspace_member(p_target_workspace_id, v_caller_id) THEN
  ...
  INSERT INTO public.workspace_entries (workspace_id, recording_id, created_at)
  ...
  IF p_delete_original THEN
    ...
    v_delete_result := public.delete_recording(p_recording_id);
...
BEGIN
  ...
  ELSE
    BEGIN
      INSERT INTO public.recordings (
        organization_id, owner_user_id, event_id, title, audio_url, video_url,
...
BEGIN
  ...
  SELECT * INTO v_source FROM public.recordings WHERE id = p_recording_id;
  ...
  IF NOT public.is_organization_member(v_source.organization_id, p_user_id) THEN
  ...
  IF NOT public.is_organization_member(p_target_org_id, p_user_id) THEN
  ...
  IF p_target_workspace_id IS NOT NULL THEN
    ...
    IF v_home_workspace_id IS NULL OR v_home_workspace_id != p_target_workspace_id THEN
      ...
      IF v_home_workspace_id IS NOT NULL THEN
        DELETE FROM public.workspace_entries
  ...
  IF p_delete_source THEN
    DELETE FROM public.transcript_chunks WHERE canonical_recording_id = p_recording_id;
    DELETE FROM public.workspace_entries WHERE recording_id = p_recording_id;

-- Phase 22 review fix follow-up: make rollup writes explicit while preserving
-- the typed landed-signal, honest grouping, and threshold-column fixes.
...
BEGIN
  ...
  task_candidates AS (
    ...
    WHERE tc.occurrence_count_30d >= tc.threshold_count
      AND tc.structural_ticket_id IS NULL
      AND NOT EXISTS (
        SELECT 1
        ...
        WHERE open_structural.type = 'task'
          ...
          AND open_structural.context->>'ticket_class_key' = tc.class_key
      )
  ),
  inserted_tasks AS (

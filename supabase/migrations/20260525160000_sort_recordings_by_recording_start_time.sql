-- Sort workspace recordings by actual recording start time before insert/sync time.
--
-- This preserves newest-first ordering across all source_app values. Imported
-- calls can be synced later than they happened, so created_at is only a fallback.

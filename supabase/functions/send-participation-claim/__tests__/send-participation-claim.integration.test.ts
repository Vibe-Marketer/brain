...
function claimDigest(label: string): string {
  return createHash('sha256').update(`${label}:${Date.now()}:${Math.random()}`).digest('hex')
}

function rpcRows(value: unknown): JsonRecord[] {
  return Array.isArray(value)
    ? value.filter((item): item is JsonRecord => typeof item === 'object' && item !== null)
    : []
}

describe.skipIf(!integrationDbReachable)('participation invitation database contract (RED)', () => {
  const prefix = `phase39-claim-db-${Date.now().toString(36)}`
  let graph: Phase39FixtureGraph
  let participantId = ''

  beforeAll(async () => {
    graph = await createPhase39FixtureGraph(prefix)
    participantId = await findParticipantId(
      graph,
      graph.events.confirmedPrimaryNeedsAction.recordingIds[0],
      graph.users.confirmedPrimary.email,
    )
  ...
  beforeEach(async () => {
    ...
    expect(deleted.error, 'reset participation invitation state').toBeNull()
  })
  ...
  it('derives owner, participant, recipient, expiry, and reminder state server-side', async () => {
    const tokenHash = claimDigest('initial')
    ...
    expect(rpcRows(created.data)[0]).toMatchObject({
      invited_email: graph.users.confirmedPrimary.email,
      reminder_opt_in: true,
      rotated: false,
    })
  ...
  it('denies non-owners, weak participants, and browser table access', async () => {
    const nonOwner = await graph.clients.unrelated.rpc('create_or_rotate_participation_claim', {
      p_participant_id: participantId,
      p_token_hash: claimDigest('non-owner'),
      p_send_one_reminder: false,
    })
    expect(nonOwner.error).not.toBeNull()

    const weakParticipant = await findParticipantId(
      graph,
      graph.events.calendarOnlyDenied.recordingIds[0],
      graph.users.calendarOnly.email,
    )
    const weak = await graph.clients.owner.rpc('create_or_rotate_participation_claim', {
      p_participant_id: weakParticipant,
      p_token_hash: claimDigest('weak'),
      p_send_one_reminder: false,
    })
    expect(weak.error).not.toBeNull()

    const browserRead = await graph.clients.owner.from('participation_claim_invitations').select('*')
  ...
  it('serializes concurrent creation and permits rotation only after seven days', async () => {
    const raced = await Promise.all([
      graph.clients.owner.rpc('create_or_rotate_participation_claim', {
        p_participant_id: participantId,
        p_token_hash: claimDigest('race-a'),
        p_send_one_reminder: false,
      }),
      graph.clients.owner.rpc('create_or_rotate_participation_claim', {
        p_participant_id: participantId,
        p_token_hash: claimDigest('race-b'),
        p_send_one_reminder: false,
      }),
    ])
    ...
    const tooSoon = await graph.clients.owner.rpc('create_or_rotate_participation_claim', {
      p_participant_id: participantId,
      p_token_hash: claimDigest('too-soon'),
      p_send_one_reminder: false,
    })
    ...
    const rotatedHash = claimDigest('rotated')
    ...
    expect(rpcRows(rotated.data)[0]).toMatchObject({ rotated: true, reminder_opt_in: false })

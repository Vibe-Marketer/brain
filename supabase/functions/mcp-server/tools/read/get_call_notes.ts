...
export const getCallNotesTool: ToolModule = {
  definition: { name: 'get_call_notes' },
  category: 'read',
  async handler({ id, params, supabase, mcpToken, corsHeaders, fetchOrgWorkspaceIds }) {
    ...
    const { data: rec } = await supabase.from('recordings').select('title').eq('id', recordingId).maybeSingle();

 ...
 */ async function handleCreateTeam(req, supabaseClient) {
  ...
  // Create the team
  const { data: team, error: teamError } = await supabaseClient.from('teams').insert({
    name: name.trim(),
  ...
  const { data: membership, error: membershipCreateError } = await supabaseClient.from('team_memberships').insert({
    team_id: team.id,
    user_id: user_id,
    role: 'admin',
    status: 'active',
  ...
  if (membershipCreateError) {
    ...
    return new Response(JSON.stringify({
      error: 'Error creating team membership',
      details: membershipCreateError.message
    }), {
      status: 500,
      headers: {
        ...corsHeaders,
 ...
 */ async function handleGetTeam(req, supabaseClient, url) {
  const teamId = url.searchParams.get('id');
  const userId = url.searchParams.get('user_id');
  // Get specific team by ID
  if (teamId) {
    const { data: team, error: teamError } = await supabaseClient.from('teams').select('*').eq('id', teamId).single();
    if (teamError || !team) {
      return new Response(JSON.stringify({
    ...
    // Get member count
    const { count: memberCount } = await supabaseClient.from('team_memberships').select('*', {
      count: 'exact',
      head: true
    }).eq('team_id', teamId).eq('status', 'active');
    // Get pending invite count
    const { count: pendingCount } = await supabaseClient.from('team_memberships').select('*', {
      count: 'exact',
      head: true
    ...
    if (userId) {
      const { data: membership } = await supabaseClient.from('team_memberships').select('*').eq('team_id', teamId).eq('user_id', userId).maybeSingle();
      if (membership) {
  ...
  if (userId) {
    ...
    const { data: memberships, error: membershipError } = await supabaseClient.from('team_memberships').select(`
        *,
        team:teams(*)
  ...
  return new Response(JSON.stringify({
    error: 'Either id or user_id query parameter is required'
  }), {
    status: 400,
    headers: {
      ...corsHeaders,
 ...
 */ async function handleUpdateTeam(req, supabaseClient, url) {
  const teamId = url.searchParams.get('id');
  const body = await req.json();
  ...
  let isAdmin = isOwner;
  if (!isOwner) {
    const { data: membership } = await supabaseClient.from('team_memberships').select('role').eq('team_id', teamId).eq('user_id', user_id).eq('status', 'active').maybeSingle();
  ...
  // Update team
  const { data: updatedTeam, error: updateError } = await supabaseClient.from('teams').update(updates).eq('id', teamId).select().single();
  if (updateError) {
    return new Response(JSON.stringify({
 ...
 */ async function handleDeleteTeam(req, supabaseClient, url) {
  const teamId = url.searchParams.get('id');
  ...
  const { data: team, error: teamError } = await supabaseClient.from('teams').select('owner_user_id').eq('id', teamId).single();
  if (teamError || !team) {
    return new Response(JSON.stringify({
      error: 'Team not found'
    }), {
  ...
  const { error: deleteError } = await supabaseClient.from('teams').delete().eq('id', teamId);
  if (deleteError) {
    return new Response(JSON.stringify({

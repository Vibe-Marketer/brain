...
/**
 * Helper: Evaluate trigger-specific conditions
 * Returns true if the trigger should fire based on its configuration
 */
export function evaluateTrigger(
  ...
  switch (triggerType) {
    case 'call_created':
      // Always fires for new calls
      return { fires: true, reason: 'Call created trigger always fires for new calls' };

    case 'transcript_phrase': {
      const pattern = triggerConfig.pattern as string;
      const matchType = (triggerConfig.match_type as string) || 'contains';
      const caseSensitive = triggerConfig.case_sensitive as boolean;
      const transcript = context.call?.full_transcript || '';

      if (!pattern) {
        return { fires: false, reason: 'No pattern configured for transcript_phrase trigger' };
      }
      ...
      switch (matchType) {
        case 'exact':
          matches = caseSensitive
            ? transcript === pattern
            : transcript.toLowerCase() === pattern.toLowerCase();
          break;
        case 'contains':
          matches = caseSensitive
        ...
        case 'regex':
          try {
            const regex = new RegExp(pattern, flags);
            matches = regex.test(transcript);
          } catch {
            return { fires: false, reason: `Invalid regex pattern: ${pattern}` };
          }
          break;
        default:
          matches = transcript.toLowerCase().includes(pattern.toLowerCase());
      }

      return {
        fires: matches,
        reason: matches
          ? `Transcript matches pattern "${pattern}" (${matchType})`
          : `Transcript does not match pattern "${pattern}" (${matchType})`,
      };
    }

    case 'sentiment': {
      const targetSentiment = triggerConfig.sentiment as string;
      const confidenceThreshold = (triggerConfig.confidence_threshold as number) || 0;
      const actualSentiment = context.call?.sentiment;
      const actualConfidence = context.call?.sentiment_confidence || 0;

      if (!targetSentiment) {
        return { fires: false, reason: 'No target sentiment configured' };
      }

      if (!actualSentiment) {
        return { fires: false, reason: 'Call sentiment not analyzed yet' };
      }

      const sentimentMatches = actualSentiment.toLowerCase() === targetSentiment.toLowerCase();
      const confidenceOk = actualConfidence >= confidenceThreshold;

      if (sentimentMatches && confidenceOk) {
        return {
          fires: true,
          reason: `Sentiment "${actualSentiment}" matches target "${targetSentiment}" with confidence ${actualConfidence.toFixed(2)} >= ${confidenceThreshold}`,
        };
      }
      ...
      return {
        ...
        reason: sentimentMatches
          ? `Confidence ${actualConfidence.toFixed(2)} below threshold ${confidenceThreshold}`
    ...
    case 'duration': {
      ...
      const minutes = triggerConfig.minutes as number;
      const actualDuration = context.call?.duration_minutes || 0;

      if (minutes === undefined) {
        return { fires: false, reason: 'No duration threshold configured' };
      }
      ...
      return {
        fires: matches,
        reason: matches
          ? `Duration ${actualDuration} minutes ${operator} ${minutes} minutes`
          : `Duration ${actualDuration} minutes does not satisfy ${operator} ${minutes} minutes`,
      };
    }

    case 'webhook':
      // Webhook trigger always fires when webhook endpoint receives valid request
      return { fires: true, reason: 'Webhook trigger fires on valid webhook request' };

    case 'scheduled':
      // Scheduled trigger fires based on scheduler, not conditions
      return { fires: true, reason: 'Scheduled trigger fires on schedule' };

    default:
      return { fires: false, reason: `Unknown trigger type: ${triggerType}` };
  }
}

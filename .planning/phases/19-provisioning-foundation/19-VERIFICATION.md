...
human_verification:
  ...
  - test: "Click Regenerate on a token row in Settings > MCP, confirm the dialog, and confirm the new token appears in the reveal dialog"
    ...
    why_human: "End-to-end UI flow with authenticated session — requires dev-browser against production or localhost"
...
| 3 | A user in settings can click "Regenerate token" and their old MCP token immediately stops working while a new one is issued | ✓ VERIFIED (code) | Full flow wired: `RiRefreshLine` button → `AlertDialog` confirmation → `regenerateToken.mutate()` → `setNewlyCreatedToken(token)` opens reveal dialog |
...
**Score:** 4/4 truths verified in code; 4 require human confirmation of live behavior
...
| `src/hooks/useMcpTokens.ts` | `useRegenerateMcpToken` hook with TanStack mutation | ✓ VERIFIED | Hook at line 98; invalidates `MCP_TOKEN_KEYS.all`, fires `onSuccess` callback |
| `src/components/settings/MCPTab.tsx` | Regenerate button in TokenRow + confirmation AlertDialog | ✓ VERIFIED | `RiRefreshLine` at line 54; `onRegenerate` prop wired at line 557; `AlertDialog` at line 686 |
...
| `MCPTab.tsx` regenerate flow | `regeneratedToken` (returned from RPC) | `regenerate_mcp_token` RPC via `regenerateMcpToken` service | Yes — atomic UPDATE RETURNING row with new hex | ✓ FLOWING |
...
Step 7b: SKIPPED for DB trigger and webhook behaviors (require live production events). Code-level wiring fully verified above.
...
| PROV-03 | 19-03 | User can regenerate MCP token (revoke old + issue new) from settings | ✓ SATISFIED | Full UI flow: `regenerateMcpToken` service → `useRegenerateMcpToken` hook → MCPTab `AlertDialog` → `TokenRevealDialog` |
...
#### 3. Token regeneration end-to-end in settings UI

**Test:** Navigate to Settings > MCP. Click the Regenerate button (refresh icon) on any token row. Confirm in the dialog. Observe the TokenRevealDialog with the new token value.
**Expected:** Confirmation dialog warns "will immediately stop working." On confirm: toast shows "MCP token regenerated," TokenRevealDialog opens with the new token. Attempting a request with the old token should fail.
**Why human:** UI interaction and visual confirmation required; old-token rejection requires a live MCP call.
...
No gaps found. All artifacts exist, are substantive (not stubs), are wired to their data sources, and the data flows produce real values. The four human verification items are behavioral end-to-end tests that require live execution — they are not code gaps.

...
- `retryFailedImport(syncJobId: string, failedExternalId: string)` — call `supabase.functions.invoke('retry-failed-import', { body: { syncJobId, failedExternalId } })`. (If a dedicated retry function doesn't exist, invoke the appropriate source's sync function with a single-call flag.) Returns success/failure.

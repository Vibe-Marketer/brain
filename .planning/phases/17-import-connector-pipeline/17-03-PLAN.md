...
6. Stage 3 — Dedup: `const externalId = \`\${file.name}-\${file.size}\``; call `checkDuplicate(supabase, user.id, 'file-upload', externalId)`. Return `{ error: 'File already imported', exists: true }` if duplicate.
7. Stage 4 — Whisper transcription:
...
8. Stage 5 — Insert via pipeline:
   ...
   ```
9. Return 200 with `{ success: true, recordingId: recording.id }`
10. Wrap the whole handler in try/catch — return 500 with error message on unexpected errors

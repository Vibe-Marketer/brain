...
REQ | TRU-01 | 30-day fix survival per fix and per category as primary metric | 19-01, 19-02, 19-03, 19-04 | COVERED | Durable per-fix fields plus persisted per-category rollup fields, rollup updater, dashboard, defer-safe denominator.
...
RESEARCH | - | Extend `runner_runs`, add `autopilot_category_trust`, add `autopilot_trust_events` | 19-01 | COVERED | Includes persisted category counters, linked schema push, persistence proof, and regenerated types.
RESEARCH | - | Use admin-only SECURITY DEFINER RPC for trust metrics | 19-01, 19-02 | COVERED | RPC is a convenience over persisted `autopilot_category_trust` rollup fields plus service/UI mapping.
...
RESEARCH | - | Exclude defers, unmerged, and unmatured runs from survival denominator | 19-01, 19-03 | COVERED | Persisted category rollup updater/RPC and runner defer branch.
...
CONTEXT | D-02 | 30-day fix survival is the primary metric | 19-01, 19-02 | COVERED | Durable per-fix and per-category survival records; Dashboard prioritizes stored survival.

...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 19-01-01 | 01 | 1 | TRU-01, TRU-02, TRU-03 | T-19-01/T-19-02/T-19-03/T-19-05 | Admin-only trust schema; persisted category survival rollups; no silent auto promotion; defers excluded from survival | static SQL | `grep -v '^--' supabase/migrations/20260613200000_phase19_autopilot_trust.sql | grep -E "autopilot_category_trust|autopilot_trust_events|autopilot_trust_metrics|rollup_autopilot_category_trust|completed_fixes_30d|survived_fixes_30d|reopened_fixes_30d|survival_rate_30d|deferred_runs_30d|last_rollup_at|survival_due_at|canary_next_run_at"` | ❌ W0 | ⬜ pending |
| 19-01-02 | 01 | 1 | TRU-01, TRU-02, TRU-03 | T-19-01/T-19-02/T-19-05 | Linked schema is live and typed before consumers run; category rollups persist in `autopilot_category_trust`, not only in an RPC response | linked Supabase | `supabase db push --linked && supabase gen types typescript --linked --schema public > src/types/supabase.ts && supabase db query --linked "select column_name from information_schema.columns where table_schema = 'public' and table_name = 'autopilot_category_trust' and column_name in ('completed_fixes_30d','survived_fixes_30d','reopened_fixes_30d','survival_rate_30d','deferred_runs_30d','last_rollup_at') order by column_name"` | ✅ | ⬜ pending |
...
| 19-02-03 | 02 | 2 | TRU-01, TRU-02, TRU-03 | T-19-09 | Admin UI makes promotion explicit and shows survival as primary metric; forbidden UI patterns are absent with a strict negated grep | type/static | `npm run type-check && test -f src/pages/admin/DashboardSection.tsx && ! grep -RnE "lucide|AI-powered|framer-motion" src/pages/admin/DashboardSection.tsx` | ✅ | ⬜ pending |
...
| 19-04-01 | 04 | 3 | TRU-01, TRU-02 | T-19-14/T-19-17 | Ladder helper fails closed; daemon refreshes stored category rollups; auto-demotion durably updates persisted counters/rung and inserts audit event | bun unit/static | `cd /Users/admin/dev/autopilot && bun test src/lib/trust.test.ts && grep -R "auto_demoted\\|autopilot_category_trust\\|autopilot_trust_events\\|completed_fixes_30d\\|survival_rate_30d\\|last_rollup_at" src/lib/trust.ts src/lib/trust.test.ts` | ❌ W0 | ⬜ pending |
| 19-04-02 | 04 | 3 | TRU-01, TRU-02, TRU-03 | T-19-14/T-19-15/T-19-16 | Approval candidates require stored auto rung plus current survival gate; no forged admin events; both admin-approved and ladder auto-approved successful merges persist originating `runner_runs` survival/canary fields (`fix_category`, `merged_at`, `survival_due_at`, `survival_status = 'pending'`, `canary_status = 'pending'`, `canary_next_run_at`) while rate-limit defers and failed/non-merged fixes do not | bun unit/static | `cd /Users/admin/dev/autopilot && bun test src/lib/approval.test.ts src/lib/trust.test.ts && grep -R "fix_category\\|merged_at\\|survival_due_at\\|survival_status\\|canary_status\\|canary_next_run_at\\|auto_approval_used" src/lib/approval.ts src/lib/approval.test.ts` | ✅ / ❌ W0 | ⬜ pending |
...
| 19-05-01 | 05 | 4 | TRU-03 | T-19-18 | Canary selects due pending merged fixes by `canary_status = 'pending' AND canary_next_run_at <= now()` and uses safe replay only | bun unit | `cd /Users/admin/dev/autopilot && bun test src/lib/canary.test.ts src/lib/evidence.test.ts` | ❌ W0 | ⬜ pending |
...
| 19-05-03 | 05 | 4 | TRU-02, ACT-02 | T-19-20/T-19-21/T-19-22 | Tier-2 obeys ladder, uses Claude-family command/model distinct from Tier-1 Codex, emits solution digest only, runs on a distinct one-cycle cadence, and forbidden digest patterns are absent with a strict negated grep | bun unit/type/static | `cd /Users/admin/dev/autopilot && bun test src/lib/tier2.test.ts src/lib/trust.test.ts src/lib/canary.test.ts && bun run typecheck && grep -R "agentCommand\\|modelFamily\\|[Cc]laude\\|Hermes" autopilot.config.ts src/lib/tier2.ts src/lib/tier2.test.ts launchd/com.callvault.autopilot-tier2.plist && grep -R "ProgramArguments\\|StartInterval\\|StartCalendarInterval" launchd/com.callvault.autopilot.plist launchd/com.callvault.autopilot-tier2.plist && test -f src/lib/tier2.ts && ! grep -RnE "stack trace|raw problem|AI-powered" src/lib/tier2.ts` | ❌ W0 | ⬜ pending |
...
- [x] Plan 19-01 verifies per-category survival persists in `autopilot_category_trust` fields (`completed_fixes_30d`, `survived_fixes_30d`, `reopened_fixes_30d`, `survival_rate_30d`, `deferred_runs_30d`, `last_rollup_at`) rather than existing only as `autopilot_trust_metrics()` output.

...
|--------|----|-----------------------|------|--------|-------|
| GOAL | phase-22 | Detect recurring classes and escalate them to structural fixes that kill the class rather than patching instances | 01, 04, 05 | COVERED | Plan 01 creates DB class/task/lifecycle contract; Plans 04-05 wire structural tier-2 handling. |
...
| REQ | REC-02 | A recurring class escalates to a structural-fix task targeting the class, not the instance | 01, 04, 05 | COVERED | Plan 01 creates idempotent internal task; Plan 04 creates class-root digest; Plan 05 queues it into tier-2 digest/admin approval and keeps it out of tier-1 claims. |
| RESEARCH | durable-table | Use durable `ticket_classes`, not view-only detection | 01 | COVERED | Migration plan includes persisted state, linked structural ticket, landed signal, baseline/post-fix anchors, and killed status. |
...
| RESEARCH | tier2-solution | Structural fixes render as solution-shaped tier-2 digest with 2-3 options | 04, 05 | COVERED | Autopilot digest tests, collection wiring, and implementation enforce tier-2/admin validation. |
| RESEARCH | tier2-event | Structural task creation/collection emits `tier2_digest_queued`, never `tier2_auto_fix_queued` | 05 | COVERED | Plan 05 Task 3 tests the exact event name and absence of auto-fix queue events for structural fixes. |
...
| CONTEXT | D-03 | Structural fixes route tier-2/admin approval only and never autonomous auto-push | 01, 03, 04, 05 | COVERED | Task shape, UI copy/action constraints, approval override, tier-2 digest event, and tier-1 exclusion all cite D-03. |
| CONTEXT | D-04 | Track per-class recurrence rate before/after and surface in AdminTab | 01, 02, 03, 05 | COVERED | DB fields/RPC/lifecycle, service/hook, UI, and cadence refresh all cover D-04. |

## Per-Task Verification Map
| Task ID | Plan | Wave | Requirement | Threat Ref | Secure Behavior | Test Type | Automated Command | File Exists | Status |
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 22-01-01 | 01 | 1 | REC-01, REC-02 | T-22-01, T-22-02, T-22-03, T-22-04 | Migration contract proves RLS/grants, namespaced class keys, service-role rollup, structural task shape, and lifecycle SQL gates | migration unit | `npm test -- src/test/migrations/phase22-ticket-classes.test.ts` | no, Wave 1 creates | pending |
| 22-01-02 | 01 | 1 | REC-01, REC-02 | T-22-01, T-22-03, T-22-04 | SQL implements threshold clustering, idempotent structural task creation, rate fields, landed signal, post-fix measurement, and killed status | migration unit | `npm test -- src/test/migrations/phase22-ticket-classes.test.ts` | no, Wave 1 creates | pending |
| 22-01-03 | 01 | 1 | REC-01, REC-02 | T-22-01, T-22-02 | Real DB proves source-namespaced collisions, idempotent rollup, landed -> post-fix rate measured -> killed lifecycle, and generated table/RPC types | integration | `supabase db push --linked --include-all && supabase gen types typescript --linked --schema public > src/types/supabase.ts && npm test -- src/test/migrations/phase22-ticket-classes.test.ts src/test/ticket-classes.integration.test.ts` | no, Wave 1 creates | pending |
...
| 22-05-01 | 05 | 3 | REC-01, REC-02 | T-22-14, T-22-15, T-22-17 | Tests prove recurrence rollup runs on tier-2 cadence, structural tasks are excluded from tier-1 claims, and structural collection emits `tier2_digest_queued` only | unit | `cd /Users/admin/dev/autopilot && npm test -- src/tier2.test.ts src/claimer.test.ts` | no, Wave 3 creates | pending |
...
| 22-05-03 | 05 | 3 | REC-02 | T-22-17 | Detected recurring class produces a tier-2 digest entry/message and never a `tier2_auto_fix_queued` event | unit + typecheck | `cd /Users/admin/dev/autopilot && npm test -- src/lib/tier2.test.ts src/tier2.test.ts src/claimer.test.ts && npm run typecheck` | yes | pending |
...
| Killed-class threshold calibration | REC-01, REC-02 | automated tests prove the lifecycle mechanics, but the exact killed threshold is an operator policy value | Review one killed-class sample and confirm the post-fix rate threshold is strict enough to mean the class has stopped generating fresh tickets. |

...
- **Storage outage limits live verification of the upload byte-path.** Verified live: bucket row (private, 5MB, mime allowlist), both RLS policies in pg_policies with exact predicates, deployed function accepting a valid attachment ref (`{"success":true,"ticketId":...}`), rejecting a foreign-prefix path with 400, and `ticket_messages.attachments` populated end-to-end. NOT verified live (blocked by the outage): an actual object landing in the bucket via the storage REST API, the RLS INSERT/SELECT enforcement at the storage data plane, and signed-URL reads. **Re-run when storage recovers:** authenticated upload to own folder (expect 200), upload to a foreign folder (expect 403), and a full UI submit via dev-browser. Logged for the verifier.

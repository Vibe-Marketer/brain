...
| 18-01-02 | 01 | 1 | SRC-01, SRC-03 | T-18-02/T-18-03 | Backfill is targeted; metrics RPC is admin-guarded | static SQL | `grep -v '^--' supabase/migrations/20260613180500_source_attribution_backfill_metrics.sql \| grep -E "ticket_source_metrics\|avg_cycle_time_hours\|reporter_id IS NULL"` | ✅ | ⬜ pending |
| 18-01-03 | 01 | 1 | SRC-01, SRC-03 | T-18-01/T-18-03 | Live schema and generated types agree | linked schema | `supabase db push --linked --include-all --yes && supabase gen types typescript --linked --schema public > src/types/supabase.ts` | ✅ | ⬜ pending |
...
| 18-04-01 | 04 | 2 | SRC-02 | T-18-12 | Source labels never expose raw enum values | unit | `npm test -- src/lib/__tests__/ticket-display.test.ts src/components/settings/__tests__/TicketTable.test.tsx` | ❌ W0 creates/extends | ⬜ pending |
| 18-04-02 | 04 | 2 | SRC-02 | T-18-13/T-18-14 | Tickets source controls filter/group without editing source | typecheck/unit | `npm run type-check` | ✅ | ⬜ pending |
| 18-05-01 | 05 | 3 | SRC-03 | T-18-15/T-18-18 | Metrics use aggregate RPC through service/hook separation | unit/typecheck | `npm test -- src/services/__tests__/admin-dashboard.service.test.ts && npm run type-check` | ✅ | ⬜ pending |
...
| CONTEXT | D-03 | Legacy/uncertain rows become unknown, never in-app user | 01 | COVERED | Targeted conservative backfill only |

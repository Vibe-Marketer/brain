...
must_haves:
  truths:
    ...
    - "All pane transitions use 500ms ease-in-out or equivalent spring physics"
  ...
  artifacts:
    - path: "/Users/Naegele/dev/callvault/src/components/layout/AppShell.tsx"
      provides: "4-pane layout component with Motion animations"
      min_lines: 80
...
<task type="auto">
  ...
  <action>
    1. Create `src/components/layout/AppShell.tsx` — the master layout component:
       ...
       - Pane 2 (Secondary): `<motion.div>` with spring width animation, bg-card/80 backdrop-blur-md rounded-2xl
       - Pane 3 (Main): `<div className="flex-1 min-w-0 bg-card rounded-2xl border border-border shadow-sm">`
       - Pane 4 (Detail): `<DetailPaneOutlet isTablet={false} />`
    ...
    2. Update `src/routes/_authenticated/index.tsx` — wire AppShell as the home page:
       - Import and render AppShell wrapping a placeholder "Calls List" main content area
       - Pass placeholder secondary pane content (folder sidebar stub)
       - Set showDetailPane to true
       - This proves the full 4-pane layout renders for authenticated users
...
2. Clicking sidebar toggle animates width with spring physics (not linear CSS transition)
3. Detail pane slides in/out with spring animation when panel is opened/closed
4. Panes have rounded-2xl corners, 12px gaps between them
5. On tablet viewport (resize to 768-1024px), sidebar auto-collapses to 72px
6. On mobile viewport (resize to <768px), only main content shows full-screen
7. Sidebar toggle is a 24x24px circular button at the edge of the sidebar
8. No framer-motion imports (all from motion/react)
</verification>

...
<task type="auto">
  ...
  <action>
    ...
    **Register resources on the McpServer:**

    The Worker must register the same resources as the stdio `src/index.ts`. Use `server.resource()` calls to register:

    1. **Operations index resource:**
       ```typescript
       server.resource(
         "operations-index",
         "callvault://operations/index",
         { description: "Complete tree of all CallVault operations", mimeType: "application/json" },
         async (uri) => ({
           contents: [{
             uri: uri.href,
    ...
    2. **Per-category resources** (loop over `operations.categories`):
       ```typescript
       for (const [catKey, cat] of Object.entries(operations.categories) as [string, any][]) {
         server.resource(
           `operations-${catKey}`,
           `callvault://operations/${catKey}`,
           { description: (cat as any).description, mimeType: "application/json" },
           async (uri) => ({
             contents: [{
               uri: uri.href,
               mimeType: "application/json",
               text: JSON.stringify(cat, null, 2),
             }]
           })
         );

         // Per-operation resources
         for (const [opKey, op] of Object.entries((cat as any).operations) as [string, any][]) {
           server.resource(
             `operations-${catKey}-${opKey}`,
             `callvault://operations/${catKey}/${opKey}`,
             { description: (op as any).description, mimeType: "application/json" },
             async (uri) => ({
               contents: [{
                 uri: uri.href,
                 mimeType: "application/json",
    ...
    3. **Register all 5 prompts** from `src/prompts.ts` using `server.prompt()`:

       The prompts to register are: `call_analysis`, `meeting_prep`, `weekly_digest`, `action_items`, `sales_pipeline`. Use the `PROMPTS` array and `getPromptMessages()` function already imported.
       ...
       for (const prompt of PROMPTS) {
         ...
         server.prompt(
           ...
           argsSchema,
           async (args) => ({
             messages: getPromptMessages(prompt.name, args as Record<string, string>),
           })
         );
       }
       ```

       This dynamically registers all 5 prompts with their argument schemas derived from the PROMPTS array, so no manual per-prompt code is needed. The `getPromptMessages()` function handles all the prompt-specific message generation.
  ...
  <verify>
    ...
    7. `grep "server.resource" /Users/Naegele/Developer/mcp/callvault-mcp/src/worker.ts` returns at least 1 match (resource registration)
    8. `grep "server.prompt" /Users/Naegele/Developer/mcp/callvault-mcp/src/worker.ts` returns at least 1 match (prompt registration)
    9. `grep "getPromptMessages" /Users/Naegele/Developer/mcp/callvault-mcp/src/worker.ts` returns at least 1 match (prompt handler wiring)
    10. `cd /Users/Naegele/Developer/mcp/callvault-mcp && npx wrangler dev --test-scheduled 2>&1 | head -5` starts without crash (may fail at runtime due to missing secrets, but should load)
  </verify>
  <done>
    Worker entry point created with: CORS for all endpoints, /.well-known/oauth-protected-resource discovery, JWT validation via jose/JWKS, per-request McpServer with all 4 meta-tools registered, resources registered (operations index + per-category + per-operation), all 5 prompts registered via server.prompt(), and proper 401 responses with WWW-Authenticate headers for OAuth auto-discovery.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **Changes needed:**
    ...
    8. **Remove the `// @ts-expect-error` comment** added in Plan 02 Task 2 -- it is no longer needed since we are now passing the context argument properly.
  ...
  <verify>
    ...
    4. `grep "ts-expect-error" /Users/Naegele/Developer/mcp/callvault-mcp/src/index.ts` returns NO matches (temporary suppression removed)
    5. The stdio server can still be started: `node /Users/Naegele/Developer/mcp/callvault-mcp/dist/src/index.js 2>&1 | head -3` (it will error about missing session, which is expected behavior)
  </verify>
  <done>
    Stdio entry point updated to create RequestContext from the old singleton pattern and pass it to the context-threaded executeOperation. Temporary @ts-expect-error from Plan 02 removed. `npm run build` succeeds. Local stdio MCP server preserved per MCP-REMOTE-08.
  </done>
</task>
...
- Resources registered: operations index, per-category, and per-operation resources
- All 5 prompts registered via server.prompt() using PROMPTS array and getPromptMessages()
...
- No @ts-expect-error comments remain in index.ts
</verification>

<success_criteria>
The Worker entry point is complete and ready for deployment. It handles the full MCP lifecycle: CORS preflight, OAuth discovery, token validation, MCP server creation with all 16 operations, resource registration, prompt registration, and proper error responses. The stdio server remains functional.
</success_criteria>

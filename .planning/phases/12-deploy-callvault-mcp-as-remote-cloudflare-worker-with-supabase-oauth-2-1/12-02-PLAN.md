...
<task type="auto">
  ...
  <action>
    ...
    **NOTE on stdio backward compatibility (MCP-REMOTE-08):**
    The existing `src/index.ts` (stdio entry) calls `executeOperation(operation, params)` without a context argument. This will now be a TypeScript error. This is intentional -- the stdio entry point (`src/index.ts`) will need to be updated in Plan 03 to create a RequestContext from the old singleton pattern and pass it. For now, the TypeScript build for the stdio server (`npm run build`) will have this type error. This is acceptable because:
    - The Worker build uses `tsconfig.worker.json` (which will point at `worker.ts`)
    - The stdio `index.ts` update is part of Plan 03's scope

    If this causes issues with Plan validation, add a temporary `// @ts-expect-error` in `index.ts` and document it for Plan 03 to remove.
  </action>
  <verify>
  ...
  <done>
    All 16 handler operations accept RequestContext. The router passes context to every handler. No handler file imports module-level singletons. Business logic unchanged across all handlers.
  </done>
</task>
...
<success_criteria>
All handler files are portable to Workers via explicit context threading. The handler code is identical in business logic to the original -- only the mechanism for obtaining the Supabase client and userId has changed from module singletons to function parameters.
</success_criteria>

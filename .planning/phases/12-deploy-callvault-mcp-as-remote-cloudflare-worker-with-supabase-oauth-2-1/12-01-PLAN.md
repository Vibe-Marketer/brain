...
<task type="auto">
  ...
  <verify>
    ...
    4. Existing `npm run build` still works (stdio server preserved)
  </verify>
  <done>
    Worker project scaffolded with wrangler.jsonc (stateless, no DO), Worker-specific tsconfig, and all new dependencies installed. Local stdio server build scripts preserved.
  </done>
</task>
...
- `tsconfig.worker.json` uses ESNext/Bundler module resolution

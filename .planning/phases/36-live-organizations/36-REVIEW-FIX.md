...
- **PRODUCTION APPLY STILL PENDING — NEEDS HUMAN AUTHORIZATION.** This new migration has **not** been applied to `vltmrnjsubfzrgrtdqey`. Per this milestone's established discipline (every other migration in this phase went through the same TEST-then-human-authorized-prod-apply gate — see Plan 06's rollout plan) and this task's explicit constraint, I did not run `supabase db push --linked` against production myself. **Action needed:** review `20260909010000_fix_merge_unclaim_rpc_row_count_guards.sql` and apply it to prod through the same authorization path used for the rest of this milestone's migrations.

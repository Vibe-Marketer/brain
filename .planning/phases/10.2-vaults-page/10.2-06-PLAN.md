...
type: execute
wave: 5
depends_on: ["10.2-04", "10.2-05", "10.2-07", "10.2-08", "10.2-09"]
files_modified:
...
must_haves:
  truths:
    - "Legacy /team route redirects to /vaults (302 redirect)"
    - "VaultListPane shows 'No vaults yet' + Create Vault CTA when bank has zero vaults"
    - "VaultDetailPane shows 'Select a vault' placeholder when no vault is selected"
    - "VaultDetailPane shows 'No recordings in this vault' when vault has zero recordings"
    - "VaultMemberPanel shows 'No members yet' + Invite button when vault has no members besides owner"
    - "ErrorBoundary wraps VaultsPage main content; thrown errors show retry + go-home fallback"
    - "Query error states (vault list, detail, recordings) show retry buttons"
    - "Settings Banks tab shows 'Vault management has moved' banner with link to /vaults"
    - "Mobile viewport (iPhone 14 Pro): vault list as overlay, detail as full-screen, member panel as bottom sheet"
  artifacts:

...
Use existing patterns from useBankContext and useFolders. All queries must have staleTime of 5 minutes. Import types from `@/types/bank`.
  ...
  <verify>npm run typecheck — no errors on new files. Verify: (1) queryKeys.vaults exists in query-config.ts with all, list, detail, members, recordings keys. (2) PanelType union includes 'vault-member' in panel.ts. (3) useVaults.ts exports useVaults, useVaultDetail, useVaultRecordings. (4) panelStore.ts handles 'vault-member' type in render switch.</verify>

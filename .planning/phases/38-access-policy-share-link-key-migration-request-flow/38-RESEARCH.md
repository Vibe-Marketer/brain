...
Use a security-definer RPC that returns only an ordinal, request state/cooldown state, and an opaque or UUID request target. It must not return owner ID/name, provider, title, transcript, summary, source IDs, or participant roster. It must suppress the entire result when any authoritative provider signal explicitly classifies the event as a webinar or when the count of distinct confirmed participant identities is at least 50. Unknown or absent provider classification is neutral under D-10 and must not suppress discovery by itself. Request creation repeats every eligibility check, including verified confirmed participation and the independent size cutoff. [VERIFIED: D-07 through D-11 and provider mapping audit]

Normalize provider classification as a tri-state value: `webinar`, `non_webinar`, or `unknown`; this preserves the distinction between an explicit non-webinar type and the absence of provider identification. Event aggregation is positive-signal based: any authoritative `webinar` value suppresses discovery, including when another copy says `non_webinar`; `unknown` contributes no positive classification and does not suppress. The 50-confirmed-participant cutoff is applied independently for every event, including unknown-only events and events with explicit non-webinar signals. [VERIFIED: provider schema/import audit and locked D-10]
...
- **AI/auto-tag webinar inference:** `WEBINAR` tags, titles, summaries, generic provider tags, and free-form meeting-type names are not authoritative provider metadata. Use the exact verified mapping below; unknown means no provider-identified webinar signal and is neutral under D-10.
...
Internal sources `file-upload`, `paste-transcript`, and `manual-mcp-import` are registered sources but are not external providers and carry no provider webinar signal. They classify as `unknown`; their presence does not suppress discovery by provider type, while verified participation and the independent 50-participant cutoff still apply. [VERIFIED: `src/config/source-registry.ts`, internal ingest source audit, and D-10]

**Resolved policy:** `webinar` is a positive, authoritative provider signal and suppresses anonymous other-copy discovery and access-request creation. `unknown` means the provider did not identify the recording as a webinar; it neither asserts `non_webinar` nor suppresses by itself. Unknown-only events and explicit-non-webinar-plus-unknown events may pass the provider-type gate, but only a verified confirmed participant may discover/request, confirmed participant count must be `< 50`, and both discovery and request RPCs must revalidate those rules server-side. Any explicit webinar signal wins over non-webinar or unknown signals. Direct grants and active share links remain available when discovery is suppressed. [VERIFIED: locked D-09 through D-11 and provider audit]

**Implementation validation task:** create checked-in payload/normalization fixtures for Zoom types `1,2,3,4,5,6,7,8,9,99`, null, malformed, and an unknown future value. Assert 5/6/9 classify as `webinar`, the documented non-webinar values classify as `non_webinar`, and null/malformed/unrecognized values classify as `unknown`. Add one fixture per other supported provider proving its current payload maps to `unknown`; Grain's fixture must prove that a free-form name containing “webinar” remains `unknown`. Add event aggregation tests proving that (a) Zoom webinar plus anything suppresses, (b) an explicit webinar/non-webinar conflict suppresses, (c) Zoom non-webinar plus unknown passes the provider gate, (d) unknown-only passes the provider gate, and (e) 49 confirmed participants may pass while 50 suppresses regardless of provider classification. Every passing-provider-gate case must still fail for an unverified, invitee-only, organization-only, or otherwise unconfirmed requester. This fixture work is a Wave 0 validation requirement, not an architectural question.
...
- [ ] Provider-event-kind normalization fixtures for every supported provider and every documented Zoom type, including malformed/unknown neutral cases and positive-webinar aggregation precedence.
...
- Participation and provider classification: HIGH — current provider schemas/mappings were audited; Zoom values are confirmed by official docs; unknown signals remain distinct and neutral while positive webinar signals and the independent 50-participant ceiling suppress discovery. Implementation fixtures remain a required proof task.

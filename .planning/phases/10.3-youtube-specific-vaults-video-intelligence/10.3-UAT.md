...
- truth: "Open a YouTube vault that has imported videos. Instead of the standard TranscriptTable, you should see a media-row list with video thumbnails (small images), title, channel name, view/like counts, duration, and category columns. An \"Outlier\" column should show a dashed placeholder with \"coming soon\" tooltip."
  ...
  severity: blocker
  test: 2
  root_cause: "youtube-import likely calls youtube-api in an environment with stale deployment and/or missing YouTube secrets, and youtube-import masks downstream failures as HTTP 400."
  artifacts:
    - path: "supabase/functions/youtube-import/index.ts"
      issue: "Downstream youtube-api non-OK responses are returned to client as generic HTTP 400"
    - path: "supabase/functions/youtube-api/index.ts"
      issue: "Missing YOUTUBE_DATA_API_KEY or TRANSCRIPT_API_KEY causes internal failure"
    - path: ".planning/debug/resolved/youtube-import-400-error.md"
      issue: "Prior confirmed auth mismatch indicates possible deploy drift"
  missing:
    - "Deploy latest youtube-import and youtube-api functions to target environment"
    - "Verify Edge Function secrets YOUTUBE_DATA_API_KEY and TRANSCRIPT_API_KEY are set"
    - "Propagate downstream status/source instead of collapsing errors to generic 400"
    - "Add regression check for internal auth forwarding to youtube-api"
...
- truth: "In the YouTube video list, click on column headers (Date, Views, Likes, Duration, Title). Each click should sort the list by that column, and an arrow indicator should show the current sort direction."
  ...
  severity: blocker
  test: 3
  root_cause: "Dependency-blocked by the same youtube-import HTTP 400 failure; sortable UI is implemented but cannot be validated without imported videos."
  artifacts:
...
- truth: "In a YouTube vault, there should be a search bar. Type a keyword and the video list should filter to matching titles after a brief debounce (~300ms)."
  ...
  severity: blocker
  test: 4
  root_cause: "Dependency-blocked by the same youtube-import HTTP 400 failure; search logic exists but cannot be exercised without imported YouTube recordings."
  artifacts:
    - path: "src/hooks/useYouTubeSearch.ts"

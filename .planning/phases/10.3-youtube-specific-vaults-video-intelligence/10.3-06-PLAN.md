...
<task type="checkpoint:human-verify" gate="blocking">
  <name>Task 3: Re-run blocked YouTube UAT validations after import unblocks</name>
  <what-built>
    youtube-import now preserves downstream error source/stage/status, forwards auth to youtube-api consistently, and has been redeployed with required secrets verified.
  </what-built>
  <how-to-verify>
    1. In Manual Import, import at least one YouTube video into a YouTube vault and confirm no opaque generic 400 blocks the flow.
    2. Open the target YouTube vault and run UAT test 3: click Date, Views, Likes, Duration, and Title headers; confirm sorting changes and arrow direction updates.
    3. Run UAT test 4: type into the YouTube search input and confirm filtering applies after ~300ms debounce.
    4. Record results in `10.3-UAT.md` and mark previously blocked tests as pass/issue with concrete evidence.
  </how-to-verify>
  <resume-signal>Type "approved" when tests 3 and 4 pass, or provide failure details to trigger additional gap planning.</resume-signal>
</task>

...
plan: 40-04
phase: 40
title: Real-DB integration tests — preservation invariants + cross-org isolation
wave: 4
depends_on: [40-01]
files_modified:
...
# Plan 40-04: Real-DB integration tests

## Objective

Verify the load-bearing preservation invariants from `40-CONTEXT.md` against a real Supabase database. NO mocks (per Phase 30 / BUG-01 — mocks let UUID/BIGINT bugs through to prod).

The edge function HTTP path requires real Fathom OAuth credentials, which we can't exercise in CI. We test the database-side contract directly: run the same UPDATE + UPSERT statements the edge function runs (extracted into a test fixture) and assert the resulting row state.

## Tasks

<task id="40-04-T01">
<action>
Create `src/test/migrations/phase40-fathom-refresh.integration.test.ts`:
...
describe.skipIf(!integrationDbReachable)(
  ...
  () => {
    ...
    it('overwrites ONLY title, full_transcript, summary, duration, recording_end_time, synced_at, source_metadata', async () => {
      ...
      expect(post!.synced_at).toBe(syncedAt);
...
Run: `npm test -- src/test/migrations/phase40-fathom-refresh.integration.test.ts`.

If the donor pattern returns no fathom recording (fresh test DB), the tests log a warning and skip — same pattern Phase 39 uses. That's acceptable; CI gates on the contract test in 40-01 plus this test running on prod when a donor exists.
</action>
<verify>
- `test -f src/test/migrations/phase40-fathom-refresh.integration.test.ts && echo OK`
- `npm test -- src/test/migrations/phase40-fathom-refresh.integration.test.ts 2>&1 | tail -30` — all 4 tests green OR cleanly skipped if no donor.
- `npx tsc --noEmit --project tsconfig.json 2>&1 | grep "phase40" || echo "no TS errors in test"`
</verify>
</task>

## Acceptance

- Test file exists with 4 `it()` blocks covering:
  1. Overwrite-only fields
  2. Preserve workspace_entries / folder_assignments / call_tag_assignments
  3. Mirror upsert idempotency
  4. Cross-org isolation (owner_user_id mismatch returns zero rows)
- Test either passes against a real DB with a donor user, or skips cleanly with a warning.
- afterAll cleanup is idempotent (re-run yields the same result).

# Requirements Archive: v2.0 Autonomous Operations
...
- [ ] **ACT-01**: Deferred — 17-05 live activation HELD by operator. Go live remains intentionally unclaimed: the kill switch is turned off and the dispatcher claims and fixes real production tickets through the existing fix→gate→approve→merge spine (first real tickets ever claimed)
...
- [ ] **ACT-03**: Deferred — 17-05 live activation HELD by operator. Rollback + blast-radius safety proven on live tickets remains intentionally held: revert path, commit-advance-by-exactly-one authority, and denylist all demonstrated under real load
...
- [x] **ACT-05**: Test-integrity gate added to the deterministic push-gate — mechanically blocks net test-deletion, assertion-weakening, and `.skip`/`.only` additions so the agent cannot "fix" by defeating the tests *(go-live blocker)*
- [x] **ACT-06**: Rebase-before-push + serialized push + repro-replay re-run on the rebased state — prevents semantically-stale merges when `main` moves between claim and push at volume *(go-live blocker)*
- [x] **ACT-07**: Worktree reaper + disk guard + wake/`caffeinate` handling so sustained-volume operation can't exhaust disk or stall on sleep
...
- [x] **QA-01**: Nightly automated QA run on schedule (the `com.callvault.qa-nightly` 03:30 launchd job + Playwright crawler already exist; wire them to the fixable-ticket path)
- [x] **QA-02**: QA failures auto-create tickets via a new `ingest_qa_ticket` RPC (`source='nightly_qa'`, DB-deduped, mirroring `ingest_sentry_ticket`) with repro/replay evidence attached
- [x] **QA-03**: Flake suppression co-ships — rerun-quarantine (a finding must fail N times before it tickets) plus an actionability gate that routes non-deterministic findings to a human-triage lane instead of the autonomous fix lane
- [x] **QA-04**: Autopilot addresses QA-sourced tickets in the same loop, severity-gated, with a per-source budget so QA churn can't starve user/Sentry tickets

### Sentry Debug → Fix → Resolve (SEN)

- [x] **SEN-03**: Sentry errors are auto-debugged via a gsd-debug-disciplined + Honcho-memory brief (session keyed by fingerprint) and routed into the autopilot fix loop *(builds on v1.0 SEN-01/02 ingestion)*
...
- [x] **SEN-05**: Sentry resolution write-back — a new `sentry-resolve` Edge Function (holding the one new secret, `SENTRY_AUTH_TOKEN`, scope `event:write`) marks the issue resolved only on a SHA-matched verified-stable deploy; a per-fingerprint fix cap freezes the category and pages on oscillation
...
|-------------|-------|--------|
| ACT-01 | Phase 17 | Deferred — 17-05 live activation HELD by operator |
| ACT-03 | Phase 17 | Deferred — 17-05 live activation HELD by operator |
| ACT-04 | Phase 17 | Done |
| ACT-05 | Phase 17 | Done |
| ACT-06 | Phase 17 | Done |
| ACT-07 | Phase 17 | Done |
| SRC-01 | Phase 18 | Done |
| SRC-02 | Phase 18 | Done |
| SRC-03 | Phase 18 | Done |
| ACT-02 | Phase 19 | Done |
| TRU-01 | Phase 19 | Done |
| TRU-02 | Phase 19 | Done |
| TRU-03 | Phase 19 | Done |
| QA-01 | Phase 20 | Done |
| QA-02 | Phase 20 | Done |
| QA-03 | Phase 20 | Done |
| QA-04 | Phase 20 | Done |
| SEN-03 | Phase 21 | Done |
| SEN-04 | Phase 21 | Done |
| SEN-05 | Phase 21 | Done |
| REC-01 | Phase 22 | Done |
| REC-02 | Phase 22 | Done |
| RSP-01 | Phase 23 | Done |
| RSP-02 | Phase 23 | Done |
| RSP-03 | Phase 23 | Done |
...
*Last updated: 2026-06-15 — v2.0 closeout status flips applied; 23 requirements done, ACT-01/ACT-03 explicitly deferred because 17-05 live activation was held by the operator.*

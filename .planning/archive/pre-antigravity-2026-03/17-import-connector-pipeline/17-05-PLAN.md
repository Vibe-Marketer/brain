...
3. Verify an "Add Source" card is visible at the end of the grid (clicking it shows future connectors)
4. Each card should show: source name, icon, status badge, call count
5. Toggle the active/inactive switch on any source — it should persist after page refresh
6. Click "Disconnect" on a connected source — verify confirm dialog appears with correct copy ("Your imported calls will be kept")
7. Drag a small MP3 or WAV file onto the file upload dropzone
8. Verify: file is accepted, transcription starts, and after a few seconds a new recording appears
9. Navigate to the calls list (/) and verify the new recording from file upload appears
10. Try uploading the same file again — should show "already imported" message (dedup working)
11. Try uploading a file >25MB — should show error message about size limit
12. If any failed imports exist, verify FailedImportsSection is visible with retry buttons
  </action>

...
must_haves:
  truths:
    ...
    - "sync_jobs rows include skipped_count tracking duplicate skips per sync"
  artifacts:
...
**Dedup skipped count tracking (locked decision):** After rewire, when `checkDuplicate` returns `isDuplicate: true`, increment a local `skippedCount` counter. At the end of the sync loop, update the `sync_jobs` row with `skipped_count: skippedCount`. If `sync_jobs` table does not have a `skipped_count` column yet, add it as part of the rewire: `ALTER TABLE sync_jobs ADD COLUMN IF NOT EXISTS skipped_count INTEGER DEFAULT 0;` — include this in the connector rewire code or as an inline migration comment for Plan 01 to pick up. This enables the UI (Plan 04) to show "Skipped N duplicates" in the toast.
...
- `skipped_count` is tracked and written to sync_jobs after each sync loop
  </verify>
  <done>

...
must_haves:
  truths:
    ...
    - "Sync edge functions (sync-meetings, zoom-sync-meetings, youtube-import) are disabled before the batch migration runs"
...
<objective>
Profile production data, disable sync edge functions, fix the migration function (COALESCE defaults + external_id), run the batch migration to completion, and verify with automated SQL + RLS checks.

Purpose: This is the core data migration work. DATA-01 (batch migration), DATA-02 (RLS verification), DATA-03 (source_metadata normalization), and DATA-05 (dry-run profiling) are all addressed here. The migration function has two known gaps that must be fixed before running: no COALESCE for NULL titles (fails NOT NULL constraint), and no external_id in source_metadata (breaks Phase 17 dedup). Per CONTEXT locked decision, all sync edge functions must be paused during the migration window to prevent new writes to fathom_calls that would be lost after archive rename.
...
<task type="checkpoint:human-action" gate="blocking">
  <name>Task 2: Disable sync edge functions before migration</name>
  <action>
...
The Supabase CLI does not support disabling edge functions — this must be done through the Dashboard UI.

Disable the following three edge functions in Supabase Dashboard:

1. Go to https://supabase.com/dashboard/project/vltmrnjsubfzrgrtdqey/functions
2. For each of these functions, click the function name, then use the "Disable" toggle or delete the deployment:
   - **sync-meetings** (Fathom sync)
   - **zoom-sync-meetings** (Zoom sync)
   - **youtube-import** (YouTube import)

IMPORTANT: These functions write to fathom_calls. If they run during or after migration, new rows written to fathom_calls will be lost when fathom_calls is renamed to fathom_calls_archive in Plan 03. Disabling them prevents data loss.

Note which functions were active vs already inactive — only active ones need re-enabling in Plan 03.
  </action>
  <verify>User confirms all three sync edge functions are disabled in the Supabase Dashboard.</verify>
  <done>sync-meetings, zoom-sync-meetings, and youtube-import edge functions are disabled. No new data can be written to fathom_calls during the migration window.</done>
</task>

<task type="auto">
  <name>Task 3: Run batch migration and verify with automated SQL + RLS checks</name>
  <!-- No file artifacts — this task runs SQL queries and edge function calls against production -->
  <action>
...
<verification>
- Sync edge functions (sync-meetings, zoom-sync-meetings, youtube-import) are disabled before migration runs
...
<success_criteria>
1. Sync edge functions disabled before batch migration starts (per CONTEXT locked decision)
2. Every non-orphaned fathom_calls row has a corresponding recordings row (unmigrated_non_orphans = 0)
3. All migrated recordings have external_id in source_metadata (missing_external_id = 0)
4. NULL titles became 'Untitled Call', NULL durations became 0
5. User A cannot query User B's recordings (RLS verified with real JWT, not service_role)
6. get_migration_progress() reports 100% complete
</success_criteria>

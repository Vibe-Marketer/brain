...
0. [Core Philosophy: The One-Click Promise](#core-philosophy-the-one-click-promise)
...
# CORE PHILOSOPHY: THE ONE-CLICK PROMISE

**This is the north star for every decision in CallVault.**
...
This philosophy is the foundation of everything we build. It supersedes technical elegance, feature richness, and implementation convenience. When in doubt, choose the path that reduces user effort.
...
Ask yourself:
- **"How many actions does this require?"** - Count clicks, keystrokes, decisions
- **"Can it be fewer?"** - Challenge every step in the workflow
- **"What's the absolute minimum?"** - Design for that, then justify any additions

### During Design & Implementation

1. **Optimize for action reduction**, not feature addition
2. **Combine steps** wherever possible (batch operations, smart defaults)
3. **Eliminate decisions** the user doesn't need to make
4. **Automate the obvious** - if we know what they want, just do it
5. **Progressive disclosure** - show only what's needed now

### In Code Review & Design Review

These questions are always valid:
- "Could this be simpler?"
- "Why does the user need to click twice here?"
- "What if we just did this automatically?"

## The Friction Test

For every feature, answer these questions:

- [ ] What is the **minimum number of actions** to accomplish the user's goal?
- [ ] Have we achieved that minimum?
- [ ] If not, **why not?** (Document the constraint - there may be valid reasons)
- [ ] Could smart defaults or automation reduce this further?

## Examples

| ❌ More Friction | ✅ Less Friction |
|------------------|------------------|
| Click to open menu → Select option → Confirm | Single click with smart default |
| Fill form → Review → Submit → Confirm | Auto-save with undo option |
| Search → Filter → Sort → Select | Smart search that anticipates intent |
| Multiple dialogs for related settings | Single unified settings panel |

## The KISS-UX Principle
...
- Simple code AND simple UX = ✅✅ Ideal
...
Every click we eliminate is a gift to our users. Every decision we remove is cognitive load we've absorbed on their behalf. This is the craft of CallVault.
...
**CRITICAL: The authoritative design system is [brand-guidelines-v4.1.md](./docs/design/brand-guidelines-v4.1.md).**

## Mandatory UI Change Protocol

**Before ANY UI/design work, you MUST:**

1. **READ** the relevant section of docs/design/brand-guidelines-v4.1.md
...
- [ ] Read relevant section of docs/design/brand-guidelines-v4.1.md
...
**When editing brand-guidelines-v4.1.md, you MUST:**
...
- **Patch** (v4.1.1 → v4.1.2): Claude does automatically for 1-2 section updates (do NOT rename file)
- **Minor** (v4.1 → v4.2): Flag to user if updating 3+ sections (RENAME file)
- **Major** (v4.x → v5.0): **NEVER** - only user decides

## When in Doubt

1. **Check docs/design/brand-guidelines-v4.1.md first**
...
- Brand style guide in `/docs/design/brand-guidelines-v4.1.md`
...
4. **Verify design compliance** - Compare against `/docs/design/design-principles-callvault.md` and `/docs/design/brand-guidelines-v4.1.md`
...
1. **Read guidelines first** - Always check brand-guidelines-v4.1.md before UI work
...
// 2. Execute research
Read("docs/design/brand-guidelines-v4.1.md") // Find metric card section

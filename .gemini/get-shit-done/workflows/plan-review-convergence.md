...
```bash
INIT=$(node "/Users/admin/dev/brain/.gemini/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
...
Set `TEXT_MODE=true` if `--text` is present in $ARGUMENTS OR `text_mode` from init JSON is `true`. When `TEXT_MODE` is active, replace every `conversational prompting` call with a plain-text numbered list and ask the user to type their choice number.

## 3. Validate Phase + Pre-flight Gate

```bash
PHASE_INFO=$(node "/Users/admin/dev/brain/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```bash
node "/Users/admin/dev/brain/.gemini/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```

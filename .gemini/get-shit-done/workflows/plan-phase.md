...
@/Users/admin/dev/brain/.gemini/get-shit-done/references/ui-brand.md
@/Users/admin/dev/brain/.gemini/get-shit-done/references/revision-loop.md
@/Users/admin/dev/brain/.gemini/get-shit-done/references/gate-prompts.md
@/Users/admin/dev/brain/.gemini/get-shit-done/references/agent-contracts.md
@/Users/admin/dev/brain/.gemini/get-shit-done/references/gates.md
</required_reading>
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@/Users/admin/dev/brain/.gemini/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
Set `TEXT_MODE=true` if `--text` is present in $ARGUMENTS OR `text_mode` from init JSON is `true`. When `TEXT_MODE` is active, replace every `conversational prompting` call with a plain-text numbered list and ask the user to type their choice number. This is required for Claude Code remote sessions (`/rc` mode) where TUI menus don't work through the Claude App.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@/Users/admin/dev/brain/.gemini/get-shit-done/references/skeleton-template.md`.
...
**If found:**
1. Read template: `/Users/admin/dev/brain/.gemini/get-shit-done/templates/VALIDATION.md`
...
SPIKE_FINDINGS_PATH=$(ls ./.gemini/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS_PATH=$(ls ./.gemini/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
```
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/Users/admin/dev/brain/.gemini/get-shit-done/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@/Users/admin/dev/brain/.gemini/get-shit-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @/Users/admin/dev/brain/.gemini/get-shit-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "/Users/admin/dev/brain/.gemini/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```

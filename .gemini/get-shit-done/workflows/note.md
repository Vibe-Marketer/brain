...
- **Global scope**: `/Users/admin/dev/brain/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/admin/dev/brain/.gemini/notes/`)
...
2. Glob `/Users/admin/dev/brain/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (/Users/admin/dev/brain/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/admin/dev/brain/.gemini/notes/` — works in any directory

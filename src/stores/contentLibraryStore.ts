...
interface ContentLibraryActions {
  ...
  fetchItems: (filters?: ContentLibraryFilters, orgId?: string | null) => Promise<void>;
  saveContentItem: (input: ContentLibraryInput, orgId?: string | null) => Promise<ContentLibraryItem | null>;
  deleteItem: (id: string) => Promise<boolean>;
  incrementItemUsage: (id: string) => Promise<void>;

  // Template actions
  fetchAllTemplates: (filters?: TemplateFilters, orgId?: string | null) => Promise<void>;
  saveNewTemplate: (input: TemplateInput, orgId?: string | null) => Promise<Template | null>;
  ...
  // Tags
  fetchTags: (orgId?: string | null) => Promise<void>;
...
export const useContentLibraryStore = create<ContentLibraryState & ContentLibraryActions>(
  (set, get) => ({
    ...
    // Content library actions
    fetchItems: async (filters?: ContentLibraryFilters, orgId?: string | null) => {
      const effectiveFilters = filters ?? get().filters;

      set({ itemsLoading: true, itemsError: null });

      const { data, error } = await fetchContentItems(effectiveFilters, orgId);
    ...
    saveContentItem: async (input: ContentLibraryInput, orgId?: string | null) => {
      ...
      const { data, error } = await saveContent(input, orgId);
    ...
    // Template actions
    fetchAllTemplates: async (filters?: TemplateFilters, orgId?: string | null) => {
      ...
      const [templatesResult, sharedResult] = await Promise.all([
        fetchTemplates(effectiveFilters, true, orgId),
        fetchSharedTemplates(effectiveFilters, orgId),
      ]);
    ...
    saveNewTemplate: async (input: TemplateInput, orgId?: string | null) => {
      ...
      const { data, error } = await saveTemplate(input, orgId);
    ...
    // Tags
    fetchTags: async (orgId?: string | null) => {
      set({ tagsLoading: true });

      const { data, error } = await getAllTags(orgId);

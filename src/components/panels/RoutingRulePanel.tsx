...
export function RoutingRulePanel({ ruleId }: { ruleId: string | null }) {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden bg-card">
          ...
          <div className="rounded-xl border border-border/50 bg-muted/20 p-4 space-y-3 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  type="button"
                  onClick={() => { if (!activeOrgId) return; bulkApply.mutate({ organizationId: activeOrgId, dryRun: false }); }}
      ...
      <footer className="shrink-0 flex items-center justify-between px-4 py-2.5 border-t border-border bg-card/50 backdrop-blur-md sticky bottom-0 z-10">
          {isEditMode ? (
            <button
              type="button"

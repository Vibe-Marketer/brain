...
- 90% rule: No card containers around content
- Remix Icon exclusively for all icons
...
- **Check vite.config.ts for server port** (default: http://localhost:8080)
- Set up the live preview environment using Playwright at the configured URL
...
- **IMPORTANT:** Check `vite.config.ts` for the server port before testing (typically 8080)
- Use `http://localhost:[PORT]` as the base URL for navigation

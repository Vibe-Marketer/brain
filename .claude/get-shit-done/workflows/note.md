...
- **Global scope**: `/Users/admin/dev/brain/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/admin/dev/brain/.claude/notes/`)
...
2. Glob `/Users/admin/dev/brain/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/admin/dev/brain/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/admin/dev/brain/.claude/notes/` — works in any directory
